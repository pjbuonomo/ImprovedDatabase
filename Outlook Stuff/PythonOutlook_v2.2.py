...
# Parsing function for email content
def parse_line(line):
    ...
    if re.match(size_first_pattern, line):
        for match in re.finditer(size_first_pattern, line):
            ...
            entries.append({"Name": name.strip(), "Size": size, "CUSIP": cusip, "Actions": action, "Price": price})
        ...
        for match in re.finditer(name_first_pattern, line):
            ...
            entries.append({"Name": name.strip(), "Size": "", "CUSIP": cusip, "Actions": action, "Price": price})
        ...
        for match in re.finditer(dual_action_pattern, line):
            ...
            entries.append({"Name": name.strip(), "Size": size, "CUSIP": cusip, "Actions": "bid", "Price": bid_price})
            entries.append({"Name": name.strip(), "Size": size, "CUSIP": cusip, "Actions": "offer", "Price": offer_price})
