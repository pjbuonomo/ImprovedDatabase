...
table <- read_excel("S:/Touchstone/Catrader/Boston/Database/CatBond/Shiny/BH_Weekly/BH20231229Clean.xlsx", col_names = TRUE, skip = 1) %>%
  na.omit() %>%
  data.frame()
...
ColNames <- sqlColumns(db_conn, "BH") %>% select('COLUMN_NAME')

result <- table %>% select(-15)
...
sqlSave(db_conn, result, tablename = "BH", rownames = FALSE, append = TRUE, verbose = TRUE)
