...
RBC_clean <- function(RBC) {
  ...
  # find the first occurrence of space from left to right, the first column is ask or bid
  ...
  # then find the first occurrence of space from right to left, 1st column = transfer to size, pay attention to "+"
  ...
  for (i in 1:nrow(RBC)) {
    ...
    Number[i] <- last_word(rest[i]) %>% str_replace("K", "k") %>% str_replace("M", "m")
  ...
  # then the rest is the name of the bond
