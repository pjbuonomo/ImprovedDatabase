...
table <- read_excel("S:/Touchstone/Catrader/Boston/Database/CatBond/Shiny/Aon_Weekly/Aon20231229Clean.xlsx", 
                    sheet = 'RLS', 
                    col_names = TRUE, 
                    skip = 1) %>% 
          data.frame()
...
table <- table %>% select(-4, -13, -14, -15)

ColNames <- sqlColumns(db_conn, "Aon") %>% select('COLUMN_NAME')
...
sqlSave(db_conn, table, tablename = "Aon", rownames = FALSE, append = TRUE, fast = FALSE)
